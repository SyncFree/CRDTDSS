%% -------------------------------------------------------------------
%%
%% mfmn_dt_gcounter: A state based, grow only, convergent counter
%%
...
%% in implementation.
%%
%% @see mfmn_kv_pncounter.erl for a counter that can be decremented
%%
...
-module(mfmn_dt_gcounter).
-behaviour(mfmn_crdt).
-export([new/0, new/2, value/1, value/2, update/3, merge/2, equal/2, vclock/1]).
...
-record (gcounter, {vclock :: vclock:vclock()}).

-opaque gcounter() :: #gcounter{}.
...
value(GCnt) ->
    VC = GCnt#gcounter.vclock,
    NodeList = vclock:all_nodes(VC),
    %io:format("~w~n",[NodeList]),
    %io:format("~w",[vclock:get_count(T, VC)]),
    CountList = [vclock:get_counter(Node, VC) || Node<- NodeList],
    lists:sum(CountList).
...
-spec update(gcounter_op(), mfmn_crdt:actor(), gcounter()) ->
                    {ok, gcounter()}.
...
equal(#gcounter{vclock=VC1},#gcounter{vclock=VC2}) ->
    vclock:equal(VC1, VC2).
    %ilists:sort(orddict:to_list(VA)) =:= lists:sort(orddict:to_list(VB)).
...
increment_by(Amount, Actor, GCnt) when is_integer(Amount), Amount > 0 ->
    %iorddict:update_counter(Actor, Amount, GCnt).
    NVC= vclock:increment(Actor, GCnt#gcounter.vclock),
    increment_by(Amount-1, Actor, GCnt#gcounter{vclock=NVC});
increment_by(_, _, GCnt) ->
    GCnt.
...
-spec vclock(gcounter()) -> vclock:vclock().
vclock(#gcounter{vclock=VC}) ->
    VC.
...
generate() ->
    ?LET(Ops, list(gen_op()),
         lists:foldl(fun(Op, Cntr) ->
                             {ok, Cntr2} = mfmn_dt_gcounter:update(Op, choose(1, 50), Cntr),
                             Cntr2
                     end,
                     mfmn_dt_gcounter:new(),
                     Ops)).
...
lots_of_actors_test() ->
    GC = lists:foldl(fun(_, GCnt) ->
                             ...
                             {ok, Cnt2} =mfmn_dt_gcounter:update({increment, Cnt}, Actor, GCnt),
                             Cnt2
                     end,
                     new(),
                     lists:seq(1, 1000)),
