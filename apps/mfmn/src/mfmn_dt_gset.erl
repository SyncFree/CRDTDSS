%% -------------------------------------------------------------------
%%
%% mfmn_dt_gset: A convergent, replicated, state based grow only set
%%
...
-record (gset, {vclock :: vclock:vclock(), set :: members()}).
...
-spec new() -> gset().
new() ->
    #gset{vclock= vclock:fresh(), set=ordsets:new()}.

-spec value(gset()) -> [member()].
value(GSet) ->
    ordsets:to_list(GSet#gset.set).
...
update({add, Elem}, Actor, GSet) ->
    NSet = ordsets:add_element(Elem, GSet#gset.set),
    NVC = vclock:increment(Actor, GSet#gset.vclock),
    {ok, #gset{set= NSet, vclock= NVC}};
update({add_all, Elems}, Actor, GSet) ->
    NSet = ordsets:union(GSet#gset.set, ordsets:from_list(Elems)),
    NVC = vclock:increment(Actor, GSet#gset.vclock),
    {ok, #gset{set= NSet, vclock= NVC}}.
...
-spec vclock(gset()) -> vclock:vclock().
vclock(GSet) ->
    GSet#gset.vclock.
